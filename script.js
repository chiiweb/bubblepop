...
function renderLeader(){
  ...
  if(gameData.leaderboard.length === 0){
    ...
    gameData.leaderboard.slice(0,5).forEach((it,i)=>{ 
      ...
      el.innerHTML += `${medal} ${it.username} — ${it.score} (lvl ${it.level})<br>`; 
    });
  }
}
...
function showUsernameScreen(){
  modal.innerHTML = `
    ...
    <button id="btnSubmitName">let's go!! 💖</button>
  `;
  modal.style.display = 'block';
...
function showStartScreen(){
  modal.innerHTML = `
    ...
    <div style="margin:12px 0; font-size:13px; text-align:left; background:rgba(0,0,0,0.15); padding:10px; border-radius:10px;">
      💡 use ${window.innerWidth > 900 ? 'A/D or Arrow Keys' : 'the joystick'} to move sideways.<br>
      ⭐ grab power-ups for bonuses!!<br>
      ⚠️ AVOID obstacles or lose lives!!<br>
      📈 game gets harder every level!!
    </div>
    <button id="btnStart">start game!! 🎮</button>
    <button id="btnShowLeader" style="margin-top:8px; background:linear-gradient(135deg, #74B9FF, #A29BFE);">leaderboard 🏆</button>
  `;
  modal.style.display = 'block';
  ...
  document.getElementById('btnShowLeader').addEventListener('click', ()=>{
    ...
    if(gameData.leaderboard.length === 0){
      leaderList.innerHTML = 'no scores yet!! be the first!! ✨';
    } else {
      gameData.leaderboard.forEach((it,i)=>{ 
        const medal = i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}.`;
        leaderList.innerHTML += `<div style="margin:6px 0;">${medal} <strong>${it.username}</strong> — ${it.score} (level ${it.level})</div>`; 
      });
    }
...
function loop(ts){
  ...
  if(!state.paused){ 
    ...
    ctx.fillText('paused!! ⏸️', window.innerWidth/2, window.innerHeight/2);
  }
